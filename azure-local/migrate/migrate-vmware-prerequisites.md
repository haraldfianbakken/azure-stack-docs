...
| Review snapshot-based backup solutions | source | If you use snapshot-based backups on VMware VMs, make sure they don’t run at the same time as Azure Migrate replication cycles. Concurrent snapshot operations can interfere with replication cycles. For more information, see [Replication cycles](/azure/migrate/concepts-vmware-agentless-migration.md#replication-cycles). |
